/*
 * Data:                2009-04-15
 * Data modyfikacji:    2019-05-19
 * Autor:               Jakub Gasior <quebes@mars.iti.pk.edu.pl>
 * Zmodyfikował:        Marcin Kurdziel
 * Zmodyfikował:        Jakub Markowski
 * Kompilacja:          gcc iptcmod.c -lip4tc -o iptcmod.run
 * Uruchamianie:        ./iptcmod.run -h
 */
...
struct rule {
    ...
    const ipt_chainlabel *defPolicy; /* domyslna polityka */
    /*
...
struct rule* parse(int argc, char ** argv) {
    ...
    while (*argv) {
        ...
        else if (!strcmp(*argv, "-P"))
        {
            ...
            argv++;
            if ((*argv == NULL) || (**argv == '-')) {
                parse_error("Expecting <default policy> after "
                            "-P <chain>\n", &error);
                break;
            }
            ...
            {
                r->defPolicy = IPTC_LABEL_ACCEPT;
            }
            else if(!strcmp(*argv, "DROP"))
            {
                r->defPolicy = IPTC_LABEL_DROP;
            }
            else
            {
                parse_error("Invalid default policy type. Only ACCEPT or DROP can be set\n", &error);
                break;
            }
...
void set_default_policy(iptc_handle_t *h, struct rule* r)
{
    int retval;
    ...
    if(!retval)
    {
        fprintf(stderr, "Chain %s is not built in. Default policy can be set only for built in chains\n", r->chain);
        exit(EXIT_FAILURE);
    }

    retval = iptc_set_policy(r->chain, r->defPolicy, NULL, h);
    if(!retval)
    {
...
int main(int argc, char **argv) {
    ...
    else if (r->operation == SET_POLICY)
    {
        /* ustawianie domyslnej polityki */
        set_default_policy(h, r);
    }
